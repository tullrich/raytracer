...
{
public:
	static std::unique_ptr<AssetReader> getReaderForFiletype(const std::string &filename);
};
...
private:
	 ...
	 */
	aiMesh* getMeshAtIndex(int index);
	 ...
	 */
	Entity::entity_ptr buildEntity(const aiNode &node, aiMatrix4x4 parentToWorldSpace);
