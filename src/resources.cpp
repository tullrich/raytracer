...
aiMesh* AssimpAssetReader::getMeshAtIndex(int index)
{
	ASSERT_ASSET_OPEN();
...
bool AssimpAssetReader::usesTextures(const aiMaterial &ai_mat)
{
	return ( ai_mat.GetTextureCount(aiTextureType_DIFFUSE) +
		...
		ai_mat.GetTextureCount(aiTextureType_NORMALS) ) > 0;
}
...
std::unique_ptr<AssetReader> ResourceLoaderFactory::getReaderForFiletype(const std::string &filename)
{

	if (std::regex_match (filename, std::regex(".*\\.obj") ))
	{
		return std::unique_ptr<AssimpAssetReader>(new AssimpAssetReader());
	}
